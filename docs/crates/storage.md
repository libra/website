...
2. Provide a response with Merkle proofs for any query that asks for a part of the
...
`T` and the root hash for the event Merkle tree generated by `T` are appended to
the accumulator.
...
Each transaction emits a list of events, which form a Merkle accumulator.
