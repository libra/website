...
* `-f` | `--faucet_server` - The host that operates a faucet service If not passed, will be derived from host parameter.
* `-a` | `—-host` - The destination host for CLI to connect with, default value is 'localhost'.
* `-p` | `-—port`- The destination port (public port of destination Libra chain) for CLI to connect with, the default value is 30307.
* `-n` | `--mnemonic_file` - The file location from which to load the mnemonic word for user account address/key generation. If not passed, a new mnemonic file will be generated.
